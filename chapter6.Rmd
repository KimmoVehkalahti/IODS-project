...
# Implement the analyses of Chapter 8 of MABS using RATS data
...
ratslf <- ratsl %>% mutate(ID = factor(ID), 
  ...
  select(ID, group, time, weight) #recorder the variables
```
...
#### 1.4.2 Check if random effect is approporiate for the data (section 1.4.2 is outside the requirement of assignment. Please go to section 1.5.1 if you're not interested)
...
Of course. I want to use the rats in data to extrapolate the rats in the world, and I want to use the types of diet to extrapolate the diets in the world (perhaps base on their trends in nutrition type or level).
...
Yes, I have multiple observations from the same diet group within a factor "ID" (rats) over time (days). There is a lack of statistical independence among this observations from same rats.

These done, I am confidently going to account for random effects(random slope) for my linear model. Before that, I need to update my hypothesis to accommodate this new variability aspect 

##### 1.4.2.3  An alternative way to test hypothesis
...
### 1.5 Testing the effect of different nutrition diet on weight of rats using two approaches

#### 1.5.1 Ajusting Baseline Approach

##### 1.5.1.1 Removing out-liers
...
##### 1.5.1.2 ANOVA to test the difference among 3 groups
...
The null hypothesis that there is no difference in means is rejected (_p_<0.001). There are some difference in the mean of weight across the rats from 3 groups. But it is still unclear which pair of group(s) have signficant difference, and post-hoc test for pairwise comparisions will be done.
...
##### 1.5.1.3 Linear regression with baseline adjusted
...
#### 1.5.2 Random intercept approach (section 1.5.2 is outside the requirement of assignment. Please skip this part and go to section 2 if you're not interested)

#### 1.5.2.1 Remove out-liers
...
#### 1.5.2.2 Fit a random intercept model
...
#### 1.5.2.3 Interclass correlation coefficient
...
#### 1.5.2.4 Display the random intercept effects
...
I further generate a density plot for the by-individual estimates of intercept. Normally, it should be normally distributed. If not, it indicates random intercept model is not a robust estimator of the effect. 
...
# Implement the analyses of Chapter 9 of MABS using BPRS data
...
Now that I have chosen a numeric variable "rating" as the dependent variable, linear model is definitely one of the preferred choices for modeling. But before a decision, I should check if there is any potential linear relationship between rating and the one of predictors "week"  (weeks when measurements were performed).
...
ggplot(bprslf, aes(x = week, y = rating, group = subject, color = subject)) +
  ...
  theme(legend.position = "none",
        ...
        axis.text = element_text(size = 10))+
  labs(title = "Fig 1.4.1(a) treatment effect over week by individual",
       x = "Time (weeks)",
       y = "BPRS rating")
```
...
bprs.group %>% 
  ...
  theme(legend.position = c(0.9,0.8),
        ...
        plot.title = element_text(size = 15,
                                  face = "bold")) + 
  labs(title = "Fig 1.4.1(b) change of rating statistics (mean±sd) over time",
       x = "Time(weeks)",
       y = "mean(bprs) +/- 2×se(bprs)")
```
...
bprslf %>% 
  ...
  theme(legend.position = "none",
        ...
        axis.text.y = element_text(size = 12),#adjust size of y axis text
        plot.caption = element_text(color = "red", size = 15))+  
  labs(title = "Fig. 1.4.2.1 (a) Change of BPRS rating by participants for treatment #1 in panel graph",
       y = "BPRS rating")
```
...
bprslf %>% 
  ...
  theme(legend.position = "none",
        ...
        axis.text.y = element_text(size = 12),#adjust size of y axis text
        plot.caption = element_text(color = "red", size = 15))+  
  labs(title = "Fig. 1.4.2.1 (b) Change of BPRS rating by participants for treatment #2 in panel graph",
       y = "BPRS rating")
```

Now according to the graph above it is clear that participants have a lot of variability in weight when starting out. For example, participant from the first and second rows of the graph started very light in weight, while rats from the third and fourth rows started heavier. This justifies an adoption of random intercept model to account for this variability. Additionally, the general trend in weight is upward over time as we would expect and the individual rat only vary very slightly in trajectory. Following the rule of parsimony, I will adopt a random intercept only model. 
