...
```{r}
#fit it
bprs_both_more <- lmer(rating ~ week + treatment + (treatment | subject) + (0 + week | subject), 
                  data = bprslf, 
                  REML = FALSE)
summary(bprs_both_more)
```


#### 2.3.3 Random intercept and slope model with interaction

```{r}
#fit it
bprs_interaction <- lmer(rating ~ week*treatment + (treatment | subject), 
                  data = bprslf, 
                  REML = FALSE)
...
```{r}
anova(bprs_intercept, bprs_both)
```
...
bprs_interaction <- lmer(rating ~   week*treatment + (treatment | subject), 
                  ...
                  REML = FALSE)
summary(bprs_interaction)
confint(bprs_interaction)
```

```{r}
severe <- bprslf %>% filter(week == 0& rating > 50) %>% pull(var = subject)
bprslf.severe <- bprslf %>% filter(subject %in% severe)

bprs_severe_both <- lmer(rating ~  week + treatment + (treatment | subject), 
                  data = bprslf.severe, 
                  REML = FALSE)
summary(bprs_severe_both)
confint.merMod(bprs_severe_both, devtol = 1e-6)
```

```{r}
mild <- bprslf %>% filter(week == 0& rating < 50) %>% pull(var = subject)
bprslf.mild <- bprslf %>% filter(subject %in% mild)

bprs_mild_both <- lmer(rating ~  week + treatment + (treatment | subject), 
                  data = bprslf.mild, 
                  REML = FALSE)
summary(bprs_mild_both)
confint.merMod(bprs_mild_both, devtol = 1e-6)
summarize_model
```

```{r}
bprs_interaction_random <- lmer(rating ~ treatment + (1|subject:treatment), bprslf)
summary(bprs_interaction_random)
confint(bprs_interaction_random)
```

```{r}
bprs_interaction_random_com <- lmer(rating ~ treatment + (1|subject:treatment) + (1|subject), bprslf)
summary(bprs_interaction_random_com)
confint(bprs_interaction_random_com)
```
```{r}
anova(bprs_interaction, bprs_intercept)
fixef(bprs_interaction)[1]+fixef(bprs_interaction)[2]+ranef(bprs_interaction)$subject
fixef(bprs_interaction)[1]
fixef(bprs_interaction)[2]
ranef(bprs_interaction)$subject
coef(bprs_interaction)
47.88556 +3.83187436	
-0.177140664	-2.628333 
-2.628333 -0.177140664	
```

```{r}

intercept <- matrix(nrow= 40, ncol = 1)

for (i in 1:40){
  data <- bprslf %>% filter(subject == i)
  fit <- lm(rating ~ week, data)
...
#generate density plot
qplot(x = `(Intercept)`, geom = 'density',  data = re)
qplot(x = value, geom = 'density',  data = df)
...
df.intercept2 <- df %>% mutate(label = "fixed")
...
```{r}
re <-  ranef(bprs_interaction)$subject
qplot(x = `(Intercept)`, geom = 'density',  data = re)
```
