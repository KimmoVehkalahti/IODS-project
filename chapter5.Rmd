...
vector.tea <- NA
for (i in 1:35){
  table.tea <- table(tea[i])
  if (min(table.tea)>=50){
    vector.tea[i] <- colnames(tea[i])
  } 
}
...
```{r}
tea <- tea %>% dplyr::select(one_of(keep.col.names))
```
...
```{r}
fviz_contrib(mca.tea, choice = "var", axes = 1, top = 15)
```
```{r}
fviz_mca_var(mca.tea, choice = "quanti.sup")
?fviz_mca_var
```
...
```{r}
a <- get_eigenvalue(mca.tea)
```

```{r}
fviz_screeplot(mca.tea, addlabels = TRUE)+
...
COS2 (squared cosine) measures the quality of the representation for variables in factor map(2-dim solution). The higher cos2, the better. Variables with low cos2 should be treated and interpreted with caution.

CO2 values for the top 10 highly-represented variables by 2-dim solution will be printed out to explore how good the variables in our data set can be represented by the first two dimensions.

```{r}
cos2.dim <- mca.tea$var$cos2 [,1:2]%>% data.frame()
cos2.dim$variable <- rownames(cos2.dim)
rownames(cos2.dim) <- NULL
cos2.dim[order(-cos2.dim[1]),c(3,1)][1:10,]
```

```{r}
cos2.dim[order(-cos2.dim[1]),c(3,2)][1:10,]
```


The printed variables are the top 10 best represented variables by 2-dim solution. Since they need to be interpreted base on relativity, a Variable Plot of MCA with high COS2 is generated.
...
```{r}
# visualize MCA
plot(mca.tea, invisible=c("ind"), habillage = "quali", graph.type = "classic")
...
empty_m <- matrix(ncol = 35, nrow = 35)
cor_matrix <- calculate_cramer(empty_m, tea.f)
cor_matrix
corrplot(cor_matrix)
```
