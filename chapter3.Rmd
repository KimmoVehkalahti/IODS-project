...
#collapse the 4 mattrices in mylist into 4 data frames.
for(w in 1:4){
...
mylist
combinations <- combn(used.predictor, 3)
combinations
all.formula.text <- apply(combinations, 2, function(x)paste("high_use~", paste(x, collapse = "+")))
all.formula.text
length(all.formula.text)
all.formula <- as.formula(all.formula.text[1])
all.formula
model <- glm(all.formula, data = alc, family = "binomial")
cv <- cv.glm(data = alc, cost = loss_func, glmfit = model, K =10)
ct.error[1,1] <- cv$delta[1]
ct.error
alc <- mutate(alc, probability = predict(model, type = "response"))
ct.error[2,1] <- loss_func(alc$high_use, alc$probability)
ct.error
mylist[[3]] <- ct.error
mylist

matrix <- matrix(unlist(mylist), ncol=6, nrow =8)
matrix1 <- mylist[[1]]
matrix1
...
all.formula.text <- apply(combinations, 
                            2, 
                            function(x)paste("high_use~"), 
                            paste(x, 
                                  collapse = "+"))
  all.formula <- as.formula(all.formula.text)
  model <- 

high_use ~ family.quality:sex + social:sex + off.campus.performance + 
    on.campus.performance

df <- data.frame(a = c("a","b","c"), b = c(1:3), c = seq(2,4))  
df$c <- NULL
df <- NULL
df
?cv.glm
...
df2 <- combn(a,4)
df2
df <- combn(a, 3, simplify=F)
df1 <- combn(a, 3)
df1
b <- apply(df1,2, function(x)paste(x, collapse = "+"))
b
b2 <- b[1]
bb <- as.formula(paste("y~",b[1]))
bb
length(b)
?paste0
?as.formula
...
```{r, echo = FALSE, eval = FALSE}
fit2 <- glm(high_use~ family.quality + social:sex + off.campus.performance + absences, data = alc, family = "binomial")
summary(fit2)
```
...
```{r, echo = FALSE, eval = FALSE}
fit_3pred <- glm(high_use ~ social:sex + off.campus.performance + 
...
```{r, echo = FALSE, eval = FALSE}
alc <- mutate(alc, probability = predict(a, type = "response"))
...
library(boot)
cv <- cv.glm(data = alc, cost = loss_func, glmfit = a, K = nrow(alc))

# average number of wrong predictions in the cross validation
cv$delta[1]
cv
cv1 <- cv.glm(data = alc, cost = loss_func, glmfit = a, K = 10)
cv1$delta[1]
```
...
a <- glm(high_use~ famrel:sex + goout+studytime+absences, data = alc, family = "binomial")
summary(a)
```
...
```{r, echo = FALSE, eval = FALSE}
data <- read.table("elections.txt", sep = ",")
vote_number_young <-  0
vote_number_old <- 0
number_candidate_young <- 0
number_candidate_old <- 0
for(i in 1:nrow(data)){
  ...
  birth_year <- data[i,4]
  if (birth_year<1980){
    vote_number_old = vote_number_old + vote
    number_candidate_old = number_candidate_old +1
  }else{ 
    vote_number_young = vote_number_young + vote
    number_candidate_young = number_candidate_young +1
  }}

print(paste0("The candidates born before 1980 get ", vote_number_old/number_candidate_old, " tickets on average"))

# Did candidates born before 1980 receive more votes than those born in 1980 or later?
```

```{r, echo = FALSE, eval = FALSE}
print(paste0("The candidates born after 1980 get ", vote_number_young/number_candidate_young, " tickets on average"))
```


```{r, echo = FALSE, eval = FALSE}
vote_number_old/number_candidate_old > vote_number_young/number_candidate_young
```
...
```{r, echo = FALSE, eval = FALSE}
alc <- alc %>% 
  mutate(family.quality = case_when
         (famrel == 4 |famrel == 5~"Good quality",
           famrel == 3~"Moderate quality",
           famrel == 2|famrel == 1~"Moderate to low quality") %>% 
           fct_rev())

alc <- alc %>% 
  mutate(family.quality = case_when
         (famrel == 5~"Excellent quality",
           famrel == 4~"Good quality",
           famrel == 3~"Moderate quality",
           famrel == 2~"Moderate to low quality",
           famrel == 1~"Low quality") %>% 
           factor(levels = c("Low quality", 
...
sex, age, Medu, studytime, reason (possible), activities, famrel, goout, health, absences




--after school activitiies``
...
(no)paid - extra paid classes within the course subject (Math or Portuguese) (binary: yes or no) 
(no)famsup - family educational support (binary: yes or no) 
...
https://www.jsad.com/doi/full/10.15288/jsad.2011.72.399
...
https://www.sciencedirect.com/science/article/pii/S0741832910001771

---friend
typical social drinking contexts were associated with men's average daily number of drinks and frequency of drunkenness 
